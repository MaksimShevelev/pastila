    ...
    return _catch$1(
      function () {
        ...
        var e = t.closest(".t-rec"),
          r,
          o = Array.from(t.querySelectorAll(".t-input-group")).map(
...
(window.t_forms__lang = (
  ...
  (window.tildaForm.validate = function (t) {
    for (
      ...
      if (!i(l)) {
        ...
        (p.obj = l),
          ...
          f && (f = parseInt(f, 10)),
          _ && (_ = parseInt(_, 10));
        var b = !y.length && !w.length,
          A = "checkbox" === g || "radio" === g,
          E = e.querySelectorAll('[name="' + h + '"]:checked').length;
        if ((((y.length && !A) || (A && E)) && (n = !1), d && (b || (A && !E))))
          p.type.push("req");
        else {
          switch (u) {
            case "email":
              (c =
                ...
                y.length && !y.match(c) && p.type.push("email");
              break;
            case "url":
              (c =
                ...
                y.length &&
                  ((m = (m =
                    (m = y.split("//")) && m.length > 1 ? m[1] : m[0]).split(
                    "/"
                  )) &&
                  m.length &&
                  m[0]
            ...
            case "phone":
              ...
              F &&
                (-1 !== F.indexOf("*")
                  ? (x += "a-zёа-я")
                  : (-1 !== F.indexOf("a") && (x += "a-z"),
                    -1 !== F.indexOf("а") && (x += "а-яё"))),
                (x += "]+$"),
                (c = new RegExp(x, "gi")),
                ((w.length && !w.match(c)) ||
                  1 == (m = w.replace(/[^0-9]+/g, "")).indexOf("000") ||
            ...
            case "number":
              (c = /^[0-9]+$/gi),
                w.length > 0 && !w.match(c) && p.type.push("number");
              break;
            case "date":
            ...
            case "time":
              (c = /^[0-9]{2}[:\.][0-9]{2}$/gi), //! TODO: 00:00 - 23:59. Заменить на /^([0-1][0-9]|2[0-3])[:\.][0-5][0-9]$/gi
                w.length > 0 && !w.match(c) && p.type.push("time");
              break;
            case "name":
              (c =
                ...
                y.length && !y.match(c) && p.type.push("name");
              break;
            case "nameeng":
              (c = /^([A-Za-z\s]{1,}((\-)?[A-Za-z\.\s](\')?){0,})*$/i),
                y.length && !y.match(c) && p.type.push("nameeng");
              break;
            case "namerus":
              (c = /^([А-Яа-яЁё\s]{1,}((\-)?[А-Яа-яЁё\.\s](\')?){0,})*$/i),
                y.length && !y.match(c) && p.type.push("namerus");
              break;
            case "string":
              (c =
                ...
                y.length && !y.match(c) && p.type.push("string");
              break;
            case "chosevalue":
              var S;
          ...
          f > 0 && y.length && y.length < f && p.type.push("minlength"),
            _ > 0 && y.length && y.length > _ && p.type.push("maxlength");
        }
    ...
    if ("y" === v) {
      ...
      if (T && window.tcart.total < window.tcart_mincntorder)
      ...
      if (e[f] && e[f].obj) {
        ...
        for (var p = 0; p < e[f].type.length; p++) {
          var y = e[f].type[p],
            w = t_forms__getMsg(y);
          (u = ""),
            (l = r.querySelector(".js-rule-error-" + y))
              ? (l.textContent && l.innerText) || !w || -1 !== c.indexOf(w)
                ? ((u = l.textContent || l.innerText),
                  -1 === c.indexOf(u) &&
                    (c = c + '<p class="t-form__errorbox-item">' + u + "</p>"))
                : (c = c + '<p class="t-form__errorbox-item">' + w + "</p>")
              : w &&
                -1 === c.indexOf(w) &&
                (c = c + '<p class="t-form__errorbox-item">' + w + "</p>"),
            i &&
      ...
      if (
        ...
        if (h && h.length) {
          var v,
            ...
            E = A ? getComputedStyle(A).zIndex : 0,
            F;
...
function t_forms__getFormData(t) {
  ...
  try {
    ...
    return (
      console.error("Failed to prepare form data for process. Error:", d),
      { status: "invalid", data: "", contentType: "" }
    );
  }
}
